...
import { Download, Plus, Wrench, CurrencyDollar, WarningCircle, MagnifyingGlass, Faders, SortAscending, PencilSimple, Trash } from "@phosphor-icons/react"
...
export default function PartsDashboardPage() {
    ...
    const inventoryValue = parts.reduce((acc, p) => acc + (p.price * p.stock), 0);
    const criticalStock = parts.filter(p => p.stock < 3).length;
        ...
        <div className="flex-1 flex flex-col h-full z-10 w-full">
            ...
            <div className="px-4 sm:px-8 py-6 sm:py-8 z-10 flex flex-col gap-8 flex-shrink-0">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
                    <div>
                        <h2 className="text-foreground font-heading font-bold text-2xl sm:text-3xl tracking-tight">Gestión de Repuestos</h2>
                        <p className="text-text-muted mt-1 text-sm font-body">Control de inventario de piezas y herramientas.</p>
                    </div>
                    <div className="flex gap-2 sm:gap-3 flex-shrink-0">
                        ...
                        <Button className="gap-2 text-xs sm:text-sm" onClick={() => window.dispatchEvent(new CustomEvent('open-product-modal', { detail: { defaultCategory: 'Repuestos' } }))}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-surface border border-surface-highlight p-6 rounded-2xl flex flex-col justify-between h-36 hover:border-text-muted transition-colors shadow-glass group">
                        <div className="flex justify-between items-start">
                            <span className="text-text-muted text-xs font-mono font-bold uppercase tracking-wider">Piezas Registradas</span>
                            <Wrench size={20} className="text-text-muted group-hover:text-foreground transition-colors" />
                        </div>
                        <div>
                    ...
                    <div className="bg-surface border border-surface-highlight p-6 rounded-2xl flex flex-col justify-between h-36 hover:border-text-muted transition-colors shadow-glass group">
                        <div className="flex justify-between items-start">
                            <span className="text-text-muted text-xs font-mono font-bold uppercase tracking-wider">Valor Repuestos</span>
                    ...
                    <div className="bg-surface border border-surface-highlight p-6 rounded-2xl flex flex-col justify-between h-36 border-l-4 border-l-critical hover:border-text-muted transition-colors shadow-glass group">
                        ...
                        <div className="flex items-baseline gap-3">
                            ...
                            <span className="text-text-muted text-xs font-body font-medium">Piezas críticas (&lt; 3 un.)</span>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div className="overflow-x-auto w-full flex-1">
                        <div className="min-w-[900px] h-full flex flex-col">
                            ...
                            <div className="grid grid-cols-12 gap-4 px-6 py-3 bg-surface-highlight/30 border-b border-surface-highlight text-xs font-mono uppercase tracking-wider text-text-muted font-bold sticky top-0">
                                <div className="col-span-4 pl-2">Pieza</div>
                            ...
                            <div className="overflow-y-auto flex-1 custom-scrollbar">
                                {filteredProducts.length === 0 ? (
                                    <div className="p-8 text-center text-text-muted">No se encontraron repuestos registrados.</div>
                                ) : (
                                    filteredProducts.map((product) => (
                                        <div key={product.id} className={`grid grid-cols-12 gap-4 px-6 py-4 border-b border-surface-highlight items-center hover:bg-surface-highlight/20 group transition-all duration-200 ${product.stock === 0 ? 'opacity-70 hover:opacity-100' : ''}`}>
                                            ...
                                            <div className="col-span-2 flex justify-end gap-2 pr-2">
                                                ...
                                                <Button variant="ghost" size="icon" className="text-critical hover:bg-critical/10" onClick={() => { if (confirm('¿Eliminar repuesto?')) deleteProduct(product.id) }}>
                                                    <Trash size={18} />
                                                </Button>
                                            </div>
                                        </div>
